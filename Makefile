...
run: ## Run the API with hot reload (LOCAL=true).
	@reflex -r '\.go$$' -s -R 'config|database|deploy|docs' -- sh -c "LOCAL=true go run main.go"
...
migrate-up: ## Apply database migrations.
	@goose -dir database/migration postgres "postgres://${POSTGRES_USER}:${POSTGRES_PASSWORD}@localhost:5432/${POSTGRES_DB}?sslmode=disable" up
...
seed-up: ## Apply database seed scripts.
	@goose -dir database/seed -table "goose_seed_db_version" postgres "postgres://${POSTGRES_USER}:${POSTGRES_PASSWORD}@localhost:5432/${POSTGRES_DB}?sslmode=disable" up
