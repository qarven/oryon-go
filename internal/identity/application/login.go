...
func (a *Application) Login(ctx context.Context, input LoginInput) (*LoginOutput, error) {
	...
	identity, err := a.repo.GetIdentityByEmail(ctx, input.Email)
	...
	credential, err := a.repo.GetIdentityCredentialByIdentityID(ctx, identity.ID, domain.IdentityCredentialTypePassword)
	if errors.Is(err, domain.ErrIdentityCredentialNotFound) {
		return nil, goerror.NewBusiness("invalid email or password", goerror.CodeUnauthorized)
	}
