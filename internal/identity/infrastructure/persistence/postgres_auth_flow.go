...
func (p *Postgres) CreateAuthFlow(ctx context.Context, flow domain.AuthFlow) error {
	...
	return p.query.CreateAuthFlow(ctx, sqlc.CreateAuthFlowParams{
		...
		IpAddress:   inetPtr(flow.IPAddress),
		UserAgent:   pgText(flow.UserAgent),
		Context:     ctxJSON,
		CreatedAt:   pgTimestamptz(flow.CreatedAt),
		ExpiresAt:   pgTimestamptz(flow.ExpiresAt),
		CompletedAt: pgTimestamptzPtr(flow.CompletedAt),
	})
}
