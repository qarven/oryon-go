...
func (p *Postgres) CreateUser(ctx context.Context, user domain.User) error {
	...
	return p.query.CreateUser(ctx, sqlc.CreateUserParams{
		...
		DeletedAt: pgTimestamptzPtr(user.DeletedAt),
	})
}
...
func (p *Postgres) ListUsers(ctx context.Context, limit, offset int32) ([]domain.User, int64, error) {
	...
	for rows.Next() {
		...
		u.DeletedAt = fromPgTimestamptz(deleted)
...
func (p *Postgres) GetIdentityCredentialByIdentityID(ctx context.Context, identityID int64, _ domain.IdentityCredentialType) (domain.IdentityCredential, error) {
	ctx, span := p.ins.Tracer("identity.persistence").Start(ctx, "GetIdentityCredentialByIdentityID")
	defer span.End()
	cred, err := p.GetPasswordCredentialByUserID(ctx, identityID)
	if err != nil {
		return domain.IdentityCredential{}, err
	}
	hash := cred.Password
	credType := domain.IdentityCredentialTypePassword
	return domain.IdentityCredential{
		ID:           cred.UserID,
		IdentityID:   cred.UserID,
		Type:         credType,
		PasswordHash: &hash,
	}, nil
}
