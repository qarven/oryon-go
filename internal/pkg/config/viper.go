...
var (
	ErrConfigTypeRequired = errors.New("config type is required")
)
...
func NewViper(pathFile string) (*Viper, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to read config file %s: %w", pathFile, err)
	}

	viperConfig.OnConfigChange(func(_ fsnotify.Event) {
		err := viperConfig.ReadInConfig()
		if err != nil {
	...
	return &Viper{v: viperConfig}, nil
}
...
func NewViperFromBytes(configType string, data []byte) (*Viper, error) {
	if strings.TrimSpace(configType) == "" {
		return nil, ErrConfigTypeRequired
	}

	viperConfig := viper.New()
	viperConfig.SetConfigType(configType)

	err := viperConfig.ReadConfig(bytes.NewReader(data))
	if err != nil {
