...
type JWT interface {
	// Issue creates a signed token.
	Issue(id string, claim Claims) (string, error)
...
func NewClaims(userID int64, userEmail string) Claims {
	return Claims{
		RegisteredClaims: jwt.RegisteredClaims{}, //nolint:exhaustruct // zero value is intentional
		UserID:           userID,
		UserEmail:        userEmail,
	}
}
